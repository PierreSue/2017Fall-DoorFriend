...
def get_args():
    desc = "Speaker Recognition on Door Friend"
    epilog = """
Examples:
    Train (enroll a list of person named person*, and mary, with wav files under corresponding directories):
    ./speaker-recognition.py -t enroll -i "./Chihwei/ ./Monmon/ ./Pierre/" -m model.out

    Predict (predict the speaker of all wav files):
    ./speaker-recognition.py -t predict -i "./*.wav" -m model.out -n name
    
    expand (To add one or more speaker to the model)
    ./speaker-recognition.py -t expand -i "./Shangde/" -m model.out -n newmodel.out
...
"""
    ...
    parser.add_argument('-t', '--task',
                       help='Task to do. Either "enroll" or "predict" or expand',
                       required=True)
...
def task_expand(input_dirs, input_model, output_model):
    m = ModelInterface.load(input_model)
    input_dirs = [os.path.expanduser(k) for k in input_dirs.strip().split()]
    dirs = itertools.chain(*(glob.glob(d) for d in input_dirs))
    dirs = [d for d in dirs if os.path.isdir(d)]
